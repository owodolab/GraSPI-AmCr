 ...
 *
 *  Author: Olga Wodo
 *  Copyright (c) 2023 Olga Wodo
 *  See accompanying LICENSE.
 *
 *  This file is part of HIERN GraSPI.
 */
...
int main(int argc, char** argv){
    ...
    if ( (argc == 1) || (infile_flag == -1) ) {
        std::cout << std::endl
        << "GraSPI5phases accepts input data in two formats:"
        << " graph, and array." << std::endl
        << "For more information check documentation"
        << std::endl << std::endl;
        std::cout << argv[0] << " -g <file.graphe> " << std::endl;
     ...
     **********************************************************************/
    graspi::graph_t*        G;
    graspi::dim_g_t         d_g(n_of_phases,0); //structure storing basic dimensions of G
    ...
    if(infile_flag == 0){
        ...
        if(n_of_phases == 5){
            bool status = graspi::build_graph_for_effective_paths(G, d_g,
                                                    vertex_colors, d_a,
        ...
        else if( !graspi::build_graph(G, d_g,
                                  vertex_colors, d_a,
                                  edge_weights, edge_colors,
                                  pixelsize, if_per) ){
            std::cout << "Problem with building graph! " << std::endl;
    ...
    if(n_of_phases == 5){
        ...
        descriptors.initiate_descriptors_5_phase();
        
        graspi::ccs_t CCs;
        
        graspi::identify_connected_components_for_effective_transport_and_compute_desc( G, vertex_colors, CCs,descriptors,d_g);
        
//        all_perfomance_indicators_5phases( descriptors,
//                                          G, d_g,
//                                          vertex_colors, d_a, edge_weights, edge_colors,
//                                          vertex_ccs, ccs,
//                                          pixelsize);
