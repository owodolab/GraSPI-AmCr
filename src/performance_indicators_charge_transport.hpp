...
namespace graspi {
    ...
    std::pair<int,int> find_useful_cc_5phases(
                                  graspi::DESC& descriptors,
                                  graspi::graph_t* G,
                                  const vertex_colors_t& C,
                                  const vertex_ccs_t& vCC,
        ...
        int n_of_orange_ccs = 0;
        int n_of_black_ccs = 0;
        int n_of_grey_ccs = 0;
        int n_of_white_ccs = 0;
        int n_of_yellow_ccs = 0;

        int n_of_orange_ccs_conn_to_top = 0;    // usefull orange components
        int n_of_black_ccs_conn_to_top = 0;    // usefull black components
        int n_of_grey_ccs_conn_to_top = 0;     // usefull grey components
        int n_of_grey_ccs_conn_to_both = 0;    // usefull grey components
        int n_of_grey_ccs_conn_to_bottom = 0;  // usefull grey components
        int n_of_white_ccs_conn_to_bottom = 0; // usefull white components
        int n_of_yellow_ccs_conn_to_bottom = 0; // usefull yellow components

        int n_of_orange_vertices_conn_to_top = 0;
        int n_of_orange_vertices = 0;
        int n_of_black_vertices_conn_to_top = 0;
        int n_of_black_vertices = 0;
        int n_of_grey_vertices = 0;
        int n_of_grey_vertices_conn_to_top = 0;
        ...
        int n_of_white_vertices = 0;
        int n_of_white_vertices_conn_to_bottom = 0;
        int n_of_yellow_vertices = 0;
        int n_of_yellow_vertices_conn_to_bottom = 0;

       
        for(unsigned int i = 0; i < CC.size(); i++){
            
            if( CC[i].color == ORANGE){
                n_of_orange_ccs++;
                n_of_orange_vertices += CC[i].size;
                if ( CC[i].if_connected_to_top() ){
                    n_of_orange_ccs_conn_to_top++;
                    n_of_orange_vertices_conn_to_top += CC[i].size;
                }
            }
            ...
            if( CC[i].color == YELLOW){
                n_of_yellow_ccs++;
                n_of_yellow_vertices += CC[i].size;
                if( CC[i].if_connected_to_bottom() ) {
                    n_of_yellow_ccs_conn_to_bottom++;
                    n_of_yellow_vertices_conn_to_bottom += CC[i].size;
                }
            }
        ...
        descriptors.update_desc("STAT_CC_Dcr",n_of_orange_ccs);
        descriptors.update_desc("STAT_CC_D",n_of_black_ccs);
        descriptors.update_desc("STAT_CC_M",n_of_grey_ccs);
        descriptors.update_desc("STAT_CC_A",n_of_white_ccs);
        descriptors.update_desc("STAT_CC_Acr",n_of_yellow_ccs);

        descriptors.update_desc("STAT_CC_Dcr_An",n_of_orange_ccs_conn_to_top);
        descriptors.update_desc("STAT_CC_D_An",n_of_black_ccs_conn_to_top);
        descriptors.update_desc("STAT_CC_M_An",n_of_grey_ccs_conn_to_top);
        descriptors.update_desc("STAT_CC_M_Ca",n_of_grey_ccs_conn_to_bottom);
        descriptors.update_desc("STAT_CC_M_AnCa",n_of_grey_ccs_conn_to_both);
        descriptors.update_desc("STAT_CC_A_Ca",n_of_white_ccs_conn_to_bottom);
        descriptors.update_desc("STAT_CC_Acr_Ca",n_of_yellow_ccs_conn_to_bottom);


        int n_all_vertices = n_of_black_vertices + n_of_white_vertices
                            + n_of_grey_vertices
                            + n_of_orange_vertices + n_of_yellow_vertices;
        descriptors.update_desc("STAT_n",n_all_vertices);
        descriptors.update_desc("STAT_n_Dcr",n_of_orange_vertices);
        descriptors.update_desc("STAT_n_D",n_of_black_vertices);
        descriptors.update_desc("STAT_n_M",n_of_grey_vertices);
        descriptors.update_desc("STAT_n_A",n_of_white_vertices);
        descriptors.update_desc("STAT_n_Acr",n_of_yellow_vertices);

        descriptors.update_desc("ABS_f_D",(double)(n_of_black_vertices+n_of_orange_vertices)
                                / (n_all_vertices));
        
        descriptors.update_desc("CT_f_conn",(double)(n_of_orange_vertices_conn_to_top+n_of_black_vertices_conn_to_top
                                                     + n_of_grey_vertices //_conn_to_either
                                                     + n_of_white_vertices_conn_to_bottom + n_of_yellow_vertices_conn_to_bottom
                                                     ) / (n_all_vertices));
        descriptors.update_desc("CT_f_conn_D_An",(double)(n_of_orange_vertices_conn_to_top+n_of_black_vertices_conn_to_top)/(n_of_black_vertices+n_of_orange_vertices));
        descriptors.update_desc("CT_f_conn_A_Ca",(double)(n_of_white_vertices_conn_to_bottom+n_of_yellow_vertices_conn_to_bottom)/(n_of_white_vertices+n_of_yellow_vertices));

        
        descriptors.update_desc("ABS_f_M",(double)n_of_grey_vertices
                                / (n_of_black_vertices+n_of_white_vertices+n_of_grey_vertices));
        
        descriptors.update_desc("CT_f_conn_DM_An",(double)(n_of_black_vertices_conn_to_top+n_of_orange_vertices_conn_to_top+n_of_grey_vertices_conn_to_top)
                                /(n_of_black_vertices+n_of_grey_vertices+n_of_orange_vertices) );
        descriptors.update_desc("CT_f_conn_AM_Ca",(double)(n_of_white_vertices_conn_to_bottom+n_of_grey_vertices_conn_to_bottom+n_of_yellow_vertices_conn_to_bottom)
                                /(n_of_white_vertices+n_of_grey_vertices+n_of_yellow_vertices));

        return std::pair<int,int>(n_of_black_vertices_conn_to_top+n_of_orange_vertices_conn_to_top,
                                  n_of_white_vertices_conn_to_bottom+n_of_yellow_vertices_conn_to_bottom);
        
    }// find_iseful_cc_5phases
...
}//compute_shortest_distance_from_source_to_target
    
    inline void
    compute_shortest_distance_from_multipleSourceC_to_targetC(
                                                      COLOR sourceC,
        ...
        vertex_t source = d_g.id(sourceC);
        connect_multiple_same_color_and_relevant_meta_vertex pred(*G,targetC,C);
        determine_shortest_distances( G, W, source, pred, d);
