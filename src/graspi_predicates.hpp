...
// consisting of one color vertices.
// This predicate identifies edges connecting vertices of the same color
class connect_multipleBOG_same_color {
public:
    connect_multipleBOG_same_color() : G_(0), vertex_colors_(0) { }
    
    connect_multipleBOG_same_color(const graspi::graph_t& G,
                       const graspi::vertex_colors_t& vertex_colors)
    : G_(&G), vertex_colors_(&vertex_colors){ }
    
    bool operator()(const graspi::edge_descriptor_t& e) const {
        
        COLOR effSColor = (*vertex_colors_)[boost::source(e, *G_)];
        COLOR effTColor = (*vertex_colors_)[boost::target(e, *G_)];
        
        COLOR sColor = (*vertex_colors_)[boost::source(e, *G_)];
        if ( ( sColor == BLACK) || ( sColor == ORANGE) || ( sColor == GREY) ) effSColor = 101;
        COLOR tColor = (*vertex_colors_)[boost::target(e, *G_)];
        if ( ( tColor == BLACK) || ( tColor == ORANGE) || ( tColor == GREY) ) effTColor = 101;

        return ( effSColor == effTColor);
    }
...
// consisting of one color vertices.
// This predicate identifies edges connecting vertices of the same color
class connect_multipleWYG_same_color {
public:
    connect_multipleWYG_same_color() : G_(0), vertex_colors_(0) { }
    
    connect_multipleWYG_same_color(const graspi::graph_t& G,
                       const graspi::vertex_colors_t& vertex_colors)
    : G_(&G), vertex_colors_(&vertex_colors){ }
    ...
    bool operator()(const graspi::edge_descriptor_t& e) const {
        ...
        COLOR sColor = (*vertex_colors_)[boost::source(e, *G_)];
        if ( ( sColor == WHITE) || ( sColor == YELLOW) || ( sColor == GREY) ) effSColor = 101;
        COLOR tColor = (*vertex_colors_)[boost::target(e, *G_)];
        if ( ( tColor == WHITE) || ( tColor == YELLOW) || ( tColor == GREY) ) effTColor = 101;
...
public:
    connect_multiple_same_color_and_relevant_meta_vertex ()
    : G_(0), vertex_colors_(0) { }
    connect_multiple_same_color_and_relevant_meta_vertex (const graph_t& G,
    ...
    bool operator()(const edge_descriptor_t& e) const {
        ...
        for (int iC=0; iC<colorsForEffectiveTransport.size(); iC++){
            if ( (*vertex_colors_)[boost::source(e, *G_)] == colorsForEffectiveTransport[iC])
                flagSource=true;
        ...
        return false; //((*vertex_colors_)[boost::source(e, *G_)] ==                (*vertex_colors_)[boost::target(e, *G_)]);
    }
...
public:
    ...
    bool operator()(const edge_descriptor_t& e) const {
        ...
        if( (*vertex_colors_)[boost::target(e, *G_)] == GREEN){
            for (int iC=0; iC<colorsForEffectiveTransport.size(); iC++){
                if ( (*vertex_colors_)[boost::source(e, *G_)] == colorsForEffectiveTransport[iC])
                    return true;
            }
