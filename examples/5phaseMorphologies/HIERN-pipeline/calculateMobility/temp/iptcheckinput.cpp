/*
 * IPTCHECKINPUT MEX-file.  See iptcheckinput.m for usage.
 *
 * Copyright 1993-2006 The MathWorks, Inc.
 * $Revision: 1.1.8.3 $  $Date: 2008/06/16 16:40:17 $
 */
 ...
 * iptcheckinput.h didn't work.  That's why we have a special version of 
 ...
 * "to be a vector."
 *
 * If you add to the attribute list, please update the attribute list in iptcheckinput.m.
 *
...
void numToOrdinal(uint32_T number, char buffer[])
{
    ...
    if (number < 1)
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:positionLessThanOne", 
                          "Argument position must be positive.");
    }
    else if (number <= ordinal_table_size)
    {
                    ...
                    const char    *message_tail)
{
    ...
    char message_fmt_alt[] = "Function %s expected %s to be %s.";
    ...
    char message_ID_fmt[] = "Images:%s:%s";
    ...
    if (mxIsEmpty(variable_position))
    {
        sprintf(message, message_fmt_alt, function_name_string, variable_name_string,
                message_tail);
    }
    else
    {
                     ...
                     const mxArray *variable_position)
{
    ...
    for (p = 0; p < num_attributes; p++)
    {
        ...
        if (! found)
        {
            mexWarnMsgIdAndTxt("Images:iptcheckinput:attributeNotFound",
...
void constructAllowedClassesString(const mxArray *class_list, char *buffer, int buffer_length)
{
    ...
    for (mwSize k = 0; k < mxGetNumberOfElements(class_list); k++)
    {
                ...
                const mxArray *variable_position)
{
    ...
    char *message_fmt;
    bool var_position_specified = !mxIsEmpty(variable_position);
    if (var_position_specified) {
      message_fmt =   
      ...
      message_fmt = 
        "Function %s expected its input %s\n"
        "to be one of these types:\n\n"
        "  %s\n\n"
        "Instead its type was %s."; 
    }    
    ...
    char message_ID_fmt[] = "Images:%s:invalidType";
    ...
    /* This conditional is necessary because numToOrdinal throws an assertion 
    for empty matrices on some platforms */
    if (var_position_specified) {
       numToOrdinal((int) mxGetScalar(variable_position), variable_position_string);
    }
    ...
    if (var_position_specified){
        ...
        sprintf(message, message_fmt, function_name_string,
            variable_name_string, allowed_classes_string, input_class);
    }
...
void checkInputs(int nrhs, const mxArray *prhs[])
{
    if (nrhs != 6)
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:numInputs",
                          "IPTCHECKINPUT requires six input arguments.");
    }

    if (!mxIsCell(prhs[1]))
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:badClassList",
                          "CLASSES must be a cell array.");
    }

    if (!mxIsCell(prhs[2]))
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:badAttributeList",
                          "ATTRIBUTES must be a cell array.");
    }

    if (!mxIsChar(prhs[3]))
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:badFunctionName",
                          "FUNCTION_NAME must be a string.");
    }

    if (!mxIsChar(prhs[4]))
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:badVariableName",
                          "VARIABLE_NAME must be a string.");
    }

    if (!mxIsNumeric(prhs[5]) || (mxGetNumberOfElements(prhs[5]) > 1))
    {
        mexErrMsgIdAndTxt("Images:iptcheckinput:badArgPosition",
                          "ARGUMENT_POSITION must be a numeric scalar or empty.");
    }
}
