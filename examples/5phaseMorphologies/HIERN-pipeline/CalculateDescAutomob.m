function [Weights]=CalculateDescAutomob (inputFile,NameFolderGraspi,NameWorkflowSave,NameFileSave,TimeStepChoice,hhh,numworkflow,PostParam)
...
%     end
% 
    k_pen_A2C = 0.7408; % penalty prefactor on mobility when travelling from amorphous to crystalline
...
%     SourceTargetPlanesDown = [2 1]; % Number of source and target planes; [1 2] for up, [2,1] for down   
    ...
    %penalty on mobility along dim 1do not move this line elsewhere !!
...
%     end  
    ...
    MobEDesc=HarmonicAverage(MobEET(2:end,:),find(MorphEET(2:end,:)==1),MorphEET(2:end,:),0.025,1);
    MobHDesc=HarmonicAverage(MobEHT(1:end-1,:),find(MorphEHT(1:end-1,:)==1),MorphEHT(1:end-1,:),0.025,1);
...
% save the profiles
    ...
    fclose(fileID);
    copyfile([NameFolderGraspi 'calculateMobility/HdepMob-Morph' NameFileSave '_sv_' num2str(TimeStepChoice(hhh)) '_wf_' num2str(numworkflow) '.txt'],[NameWorkflowSave 'HdepMob-Morph' NameFileSave '_sv_' num2str(TimeStepChoice(hhh)) '.txt'])
    save([NameWorkflowSave 'MobEHT_' inputFile(1:end-4) '.mat'],'MobEHT','HdepMobEHT','HEffdepMobEHT')
    save([NameWorkflowSave 'MobEET_' inputFile(1:end-4) '.mat'],'MobEET', 'HdepMobEET', 'HEffdepMobEET')
end
